...
/**
 * Função para filtrar anotações com base no termo de pesquisa
 */
function filterNotes(searchTerm) {
    ...
    if (!searchTerm) return studentNotes;
    
    const lowerCaseSearchTerm = searchTerm.toLowerCase();
    
    return studentNotes.filter(note => {
        const titleMatch = note.title && note.title.toLowerCase().includes(lowerCaseSearchTerm);
        const contentMatch = note.content.toLowerCase().includes(lowerCaseSearchTerm);
        const topicMatch = note.topic && note.topic.toLowerCase().includes(lowerCaseSearchTerm);
...
/**
 * Atualiza a lista de anotações exibidas com base no filtro
 */
function updateNotesList(filteredNotes) {
    const sidebarList = document.getElementById('sidebar-notes-list');
    
    sidebarList.innerHTML = filteredNotes.length > 0
    ...
    // Re-adiciona os event listeners para as anotações filtradas
    document.querySelectorAll('.sidebar-note').forEach(note => {
        note.addEventListener('click', function() {
            const noteId = parseInt(this.getAttribute('data-id'));
            if (!isNaN(noteId)) {
...
/**
 * Modifique a função loadNotes para usar o filtro
 */
...
// Adicione o event listener para o campo de pesquisa
